...
public:
   ...
   {
       if(index == word.size()) return true;     // word exists in matrix   
       if(x < 0 or y < 0 or x >= board.size() or y >= board[0].size() or board[x][y] == '.' or board[x][y] != word[index]) return false; // boundary check + visited check
       char temp = board[x][y];
